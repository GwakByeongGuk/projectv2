...
router.post('/admin-dashboard', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get("http://localhost:8000/admin/admin-dashboard");
...
router.post("/visitor-register", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post("http://localhost:8050/visitor/visitor-register", params, {
      headers: {
...
router.get("/visitor-dashboard", async (req, res) => {
  try {
    const response = await axios.get("http://localhost:8050/visitor/visitor-dashboard");
...
router.post("/admin-approve/:visitorId", async (req, res) => {
  ...
  try {
    // FastAPI에 승인 요청 보내기
    const response = await axios.post(`http://localhost:8000/admin/admin-approve/${visitorId}`);
...
router.post("/admin-reject/:visitorId", async (req, res) => {
  ...
  try {
    // FastAPI에 거절 요청 보내기
    const response = await axios.post(`http://localhost:8000/admin/admin-reject/${visitorId}`);
    if (response.data.success) {
      res.redirect('/admin-dashboard');
    } else {
      res.status(500).send(response.data.message);
...
router.post("/admin-exit/:visitorId", async (req, res) => {
  ...
  try {
    // FastAPI에 퇴입 요청 보내기
    const response = await axios.post(`http://localhost:8000/admin/exit/${visitorId}`, {
      admin_id: 1
    });
